...
atomicGLGodRay = function(nname, agl,fragmentShaderID, vertexShaderID,nnbTex,nnbLights){
	// attributes
	// -------------------------------------------------
	// name
	this.name = nname ;
    ...
    this.createProgram =  function (agl,fragmentShaderID, vertexShaderID) {
        ...
        for (i = 0; i < this.nbLight; i++) { 
			...
			this.pointLightLocationUniform[i] = agl.gl.getUniformLocation(program, "uPointLightPosition"+i);
				...
				//this.volumUniform = agl.gl.getUniformLocation(program, "uVolum");
    ...
    this.setUniforms = function(aGL,aMS){
    	...
    	// matrix
			...
			//aGL.gl.uniform1f(this.timeUniform,this.wVolum) ;
